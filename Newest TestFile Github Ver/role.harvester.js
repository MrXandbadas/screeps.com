...
run: function (creep) {
    ...
    if (creep.memory.jobTask[0].harvestingSource == undefined || null ) {
        creep.creep.memory.jobTask. .harvestingSource = true
    };
       ...
       if (creep.memory.jobTask[0].harvestingSourceID == undefined) {
        ...
        if (creep.harvest(source) == ERR_NOT_IN_RANGE) {
            const path = creep.pos.findPathTo(source);
                    ...
                    creep.moveByPath(Memory.path, {visualizePathStyle: {stroke: '#ffffff'}});
